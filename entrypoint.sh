...
case "$SERVICE_TYPE" in
  api)
    ...
    exec uvicorn app.v1.main:app --host 0.0.0.0 --port "${PORT:-8000}" --workers 2
    ;;
  worker)
    echo "Starting Celery worker..."
    exec celery -A app.v1.tasks.celery_app worker \
      --loglevel=info \
      --pool=threads \
      --concurrency=4
    ;;
  beat)
    ...
    exec celery -A app.v1.tasks.celery_app beat --loglevel=info
    ;;
  *)
