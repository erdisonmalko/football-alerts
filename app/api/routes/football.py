...
async def list_leagues(_: User = Depends(get_current_user)):
    ...
    logger.info("Fetching list of all leagues.")
...
async def list_teams(
    ...
    if league_code.upper() not in valid_codes:
        logger.warning(f"Attempt to fetch teams for unsupported league code: {league_code}")
    ...
    logger.info(f"Retrieved teams for league: {league_code}")
    return teams
